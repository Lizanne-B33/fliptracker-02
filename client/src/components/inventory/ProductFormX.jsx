...
const ProductForm = ({ item, onSaved, endpoint }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key === 'prod_image') {
          ...
          if (value instanceof File) {
            fd.append('prod_image', value);
            console.log('prod_image is a File:', value);
          } else {
            console.log('prod_image is NOT a File:', value);
          }
      ...
      // submitFormData call here...
    } catch (err) {
      console.error(err);
    }

    // Determine method & endpoint
    const method = item ? 'PATCH' : 'POST';
    const url = item ? `/api/inventory/product/${item.id}/` : endpoint;
    ...
    await submitFormData(
      ...
      onSaved,
      method,
      true // important: send as multipart/form-data
    );
  } catch (err) {
